...
Self-contained example idiom: pick a case from `Locals` and dispatch via
`run_local`. Run from this folder with `goal_based_allocation` installed:
...
def run_local(local: Locals) -> None:
    ...
    elif local == Locals.REGIME_SWITCH:
        meta, df = read_chain(os.path.join(DATA_DIR, TENORS[1]['csv']))
        vs = build_otm_slice(meta, df, source=PriceSource.LAST, drop_strikes=[1500.0])
...
if __name__ == '__main__':
    run_local(local=Locals.TERM_STRUCTURE)
