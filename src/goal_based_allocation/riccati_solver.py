...
if __name__ == "__main__":
    from regime_switch_paper import create_paper_assets
    ...
    eq = assets['equity']
    T = 10.0; r = 0.02; c = 0.03

    print("\n--- Test 1: Riccati ODE ---")
    ell, ric = find_ell(eq, T, 0.05, r, c)
    print(f"  ℓ = {ell:.4f}")
    print(f"  a(T): [{ric.a[0,-1]:.6f}, {ric.a[1,-1]:.6f}]")
    print(f"  E[Π_T] = {ric.expected_wealth(0):.4f}  (target: {eq.pi0*np.exp(0.05*T):.4f})")
    print(f"  min(a): [{np.min(ric.a[0]):.6f}, {np.min(ric.a[1]):.6f}]")

    d0 = ric.derived_at_tau(T)
    print(f"  μ (diffusion drift): [{ric.dp['mu_bar'][0]:.6f}, {ric.dp['mu_bar'][1]:.6f}]")
    print(f"  r_h = {ric.r_h}, r_c = {ric.r_c}")
    print(f"  ω*_a(t=0): [{d0['w_a'][0]:.4f}, {d0['w_a'][1]:.4f}]")
    print(f"  Π*(t=0):   [{d0['Pi_star'][0]:.4f}, {d0['Pi_star'][1]:.4f}]")
    print(f"  ω*(t=0, Π=100, reg=0) = {ric.omega_star(0, 100, 0):.4f}")

    print("\n--- Test 2: MC (100K paths) ---")
    mc = simulate_mv_optimal(ric, n_paths=100_000, seed=42)
    surv_mc = np.mean(mc['survived'])
    print(f"  Survival: {surv_mc:.4f}")
    print(f"  E[Π_T] (all): {np.mean(mc['Pi_T']):.4f}  (target: {ric.expected_wealth(0):.4f})")
    if surv_mc > 0.01:
        print(f"  E[Π_T|surv]: {np.mean(mc['Pi_T'][mc['survived']]):.4f}")
        print(f"  std[Π_T|surv]: {np.std(mc['Pi_T'][mc['survived']]):.4f}")
    ...
    if gap.x0 > 0 and gap.x0 < 20:
        surv_gap = compute_survival(T, gap.x0, gap)
        print(f"  Analytic surv (gap): {surv_gap:.4f}")
