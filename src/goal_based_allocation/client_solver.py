...
if __name__ == "__main__":
    ...
    for p in profiles:
        print(f"\n{'='*75}")
        res = solve_client(p, n_grid=8, verbose=True)
        results.append(res)
        print()
    ...
    if not valid:
        ...
        print("SUMMARY")
        print("="*75)
        headers = [r['profile'].name for r in valid]
        print(f"{'':>18}" + "".join(f"{h:>16}" for h in headers))
        print("-" * 66)

        rows = [
            ('c', [r['profile'].c for r in valid], '%'),
            ('g', [r['profile'].g for r in valid], '%'),
            ('r_h', [r['r_h'] for r in valid], '%'),
            ('q_dd', [r['profile'].q_dd for r in valid], 'f1'),
            ('Bd/Eq/PE', None, None),
            ('k', [r['k'] for r in valid], 'f2'),
            ('σ_unc', [r['sig_unc'] for r in valid], '%'),
            ('x_25 (=L_T)', [r['x_25_input'] for r in valid], 'f1'),
            ('q25 achieved', [r['q25'] for r in valid], 'f1'),
            ('q50 target', [r['profile'].x_50 for r in valid], 'f0'),
            ('q50 achieved', [r['q50'] for r in valid], 'f1'),
            ('q75 target', [r['profile'].x_75 for r in valid], 'f0'),
            ('q75 achieved', [r['q75'] for r in valid], 'f1'),
            ('q90', [r['q90'] for r in valid], 'f1'),
            ('E[Π_T]', [r['mu'] for r in valid], 'f1'),
            ('Π*_T', [r['Pi_star_T'] for r in valid], 'f0'),
            ('Survival', [r['surv'] for r in valid], '%'),
            ('|ω*_a|', [r['w_a'] for r in valid], 'f3'),
        ]
        for label, vals, fmt in rows:
            if vals is None:
                ws = [f"{r['w_bd']:.0%}/{r['w_eq']:.0%}/{r['w_pe']:.0%}"
                      for r in valid]
                print(f"{'Bd/Eq/PE':>18}" + "".join(f"{w:>16}" for w in ws))
                continue
            s = f"{label:>18}"
            for v in vals:
                if fmt == '%': s += f"{v:15.1%}"
                elif fmt == 'f0': s += f"{v:16.0f}"
                elif fmt == 'f1': s += f"{v:16.1f}"
                elif fmt == 'f2': s += f"{v:16.2f}"
                elif fmt == 'f3': s += f"{v:16.3f}"
            print(s)
